...
class SelectSqlGenerationTest extends OrmTestCase
{
    ...
    public function testSupportsOrderByStartingWithFunction()
    {
        $this->assertSqlGeneration(
            'SELECT u FROM Doctrine\Tests\Models\Forum\ForumUser u ORDER BY LENGTH(u.username) asc',
            'SELECT f0_.id AS id_0, f0_.username AS username_1 FROM forum_users f0_ ORDER BY LENGTH(f0_.username) ASC'
        );
    }

    public function testSupportsOrderByWithSimpleArithmeticExpression()
    {
        $this->assertSqlGeneration(
